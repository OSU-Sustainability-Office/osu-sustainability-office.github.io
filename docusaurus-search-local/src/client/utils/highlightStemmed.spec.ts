import { MetadataPosition, ChunkIndexRef, HighlightChunk } from '../../shared/interfaces';
...
describe('highlightStemmed', () => {
  ...
  ])("highlightStemmed('%s', %j, %j, %j) should return '%s'", (text, positions, tokens, maxLength, result) => {
    expect(highlightStemmed(text, positions, tokens, maxLength)).toEqual(result);
  });
});
...
describe('splitIntoChunks', () => {
  ...
  ])("splitIntoChunks('%s', %j, %j, 0, 0) should return %j", (text, positions, tokens, chunks, chunkIndex) => {
    const chunkIndexRef = {} as ChunkIndexRef;
    expect(splitIntoChunks(text, positions, tokens, 0, 0, chunkIndexRef)).toEqual(chunks);
