...
import { __setLanguage, __setRemoveDefaultStopWordFilter, __setRemoveDefaultStemmer } from './proxiedGenerated';
...
describe('smartQueries', () => {
  ...
  test.each<[string[], TestQuery[]]>([
    ...
    expect(smartQueries(tokens, zhDictionary).map(transformQuery)).toEqual(queries);
  });
});
      ...
      .map(
        (item) =>
          `${item.presence === lunr.Query.presence.REQUIRED ? '+' : ''}${
            (item.wildcard & lunr.Query.wildcard.LEADING) === lunr.Query.wildcard.LEADING ? '*' : ''
          }${item.value}${(item.wildcard & lunr.Query.wildcard.TRAILING) === lunr.Query.wildcard.TRAILING ? '*' : ''}`
      )
      .join(' '),
  };
}
