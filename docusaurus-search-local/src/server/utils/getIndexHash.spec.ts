...
(klawSync as jest.MockedFunction<typeof klawSync>).mockImplementation((root, options) => {
  let files: string[] = [];
  if (root === '/tmp/docs') {
    files = ['/tmp/docs/a.md', '/tmp/docs/b.md', '/tmp/docs/b.png'];
  }
  return files.map((path) => ({ path } as klawSync.Item)).filter(options.filter);
});
...
(fs.lstatSync as jest.MockedFunction<typeof fs.lstatSync>).mockImplementation((filePath: string) => {
  return {
...
describe('getIndexHash', () => {
  test.each<[Partial<ProcessedPluginOptions>, string | null, number]>([
    ...
    [{ hashed: true, indexDocs: true, docsDir: ['/does-not-exist/docs'] }, null, 1],
